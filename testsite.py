...
proc_signal_clean = channel.process(mod_signal)
...
fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1, figsize=(14, 8), gridspec_kw={'height_ratios': [1, 1]})

time_axis = np.arange(len(proc_signal))
ax2.plot(time_axis, proc_signal, color='#1f77b4', linewidth=1)

ax2.set_title(f"Канал передачи (Anomaly: {anomaly}, SNR: {snr}dB)")
ax2.set_ylabel("Амплитуда")
ax2.legend(loc="upper right")
ax2.grid(True, alpha=0.3)

ax1.plot(time_axis, proc_signal_clean, color='#1f77b4', linewidth=1)
ax1.set_title(f"Канал передачи (Аномалия: none, SNR: {snr}dB)")
